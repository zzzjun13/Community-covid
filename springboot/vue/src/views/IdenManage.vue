  ...
  <div>
    <el-card class="box-card">
      ...
      <el-table v-loading="loading" size="small" :data="userList" border style="width: 100%;" height="420">
        ...
        <el-table-column prop="sex" :formatter="showSex" label="性别" width="70">
        ...
        <el-table-column prop="statas" label="是否确诊" width="150"></el-table-column>
        <el-table-column prop="dizhi" label="地址" width="150"></el-table-column>
        ...
        <el-table-column label="操作" >
          <template slot-scope="scope">
            <el-button   v-hasPermission="'user:edit'" size="medium" type="primary" icon="el-icon-edit-outline" @click="edit(scope.row.id)"></el-button>
            <el-button v-hasPermission="'user:delete'" type="danger" size="medium"  icon="el-icon-delete" @click="del(scope.row.id)"></el-button>
            <el-tooltip
                class="item"
                effect="dark"
                size="medium"
      ...
      <!-- 添加对话框 -->
      <el-dialog title="登记名单" @close="closeDialog" :visible.sync="addDialogVisible" width="50%;">
        <!-- 表单 -->
        <span>
            <el-form
                :model="addForm"
          ...
          <el-input v-model="addForm.statas" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="就诊日期" :label-width="formLabelWidth">
          <el-date-picker v-model="addForm.idate"
                          align="right"
                          type="date"
                          placeholder="选择日期"
      ...
      <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="50%" @close="editClose">
        <span>
          ...
          >
            ...
            <el-form-item label="是否确诊">
          <el-input v-model="editForm.statas" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="就诊日期" :label-width="formLabelWidth">
          <el-date-picker v-model="editForm.idate"
                          align="right"
                          type="date"
                          placeholder="选择日期"
                          :picker-options="pickerOptions">
...
export default {
  ...
  methods: {
    ...
    async assignRoles(id) {
      ...
      const { data: res } = await this.$http.get("system/roles/" + id + "/roles");
      if (res.success) {
        this.roles = res.data.idRoles.roles;
        this.value = res.data.idRoles.values;
        this.uid = id;
        setTimeout(() => {
          loading.close();
    ...
    async doAssignRoles() {
      ...
      const { data: res } = await this.$http.post(
          "system/users/" + this.uid + "/assignRoles",
          this.value
      );
      if(res.success){
        this.$notify.success({
     ...
     */
    async getUserList() {
      const { data: res } = await this.$http.get("system/users/findUserList", {
    ...
    async addUser() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) {
          ...
          this.btnLoading = true;
          this.btnDisabled = true;
          const { data: res } = await this.$http.post("system/users/add", this.addForm);
          if(res.success){
            this.$notify.success({
              title:'操作成功',
              message:'用户添加成功',
     ...
     */
    async edit(id) {
      const { data: res } = await this.$http.get("system/users/edit/" + id);
      if(res.success){
    ...
    async changUserStatus(row) {
      const { data: res } = await this.$http.put(
          "system/users/updateStatus/" + row.id + "/" + row.status
      );
      if(!res.success){
